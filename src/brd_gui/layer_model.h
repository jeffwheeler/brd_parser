...
const std::vector<Layer> kG1Layers = {{
    {0x01, 0xEA, nullptr},
    {0x01, 0xED, nullptr},
    {0x01, 0xEE, nullptr},
    {0x01, 0xF0, "Silk bot"},
    ...
    {0x01, 0xF9, "Fab dims?"},
    {0x01, 0xFB, "Assy?"},
    {0x01, 0xFD, "Design outline?"},
}};

const std::vector<Layer> kG3Layers = {
    {0x3, 0xFA, nullptr},
    {0x3, 0xFC, nullptr},
    {0x3, 0xFD, nullptr},
};

const std::vector<Layer> kG4Layers = {
    {0x04, 0xF9, nullptr},       {0x04, 0xFA, nullptr}, {0x04, 0xFB, nullptr},
    {0x04, 0xFC, "Title block"}, {0x04, 0xFD, nullptr},
};

// Used so that etch can be given a group name
const std::vector<Layer> kG6Layers = {};

const std::vector<Layer> kG7Layers = {
    {0x07, 0xF0, nullptr}, {0x07, 0x12, nullptr}, {0x07, 0x13, nullptr},
    {0x07, 0xF7, nullptr}, {0x07, 0xF8, nullptr}, {0x07, 0xFD, nullptr},
};

const std::vector<Layer> kG9Layers = {
    {0x09, 0xEC, nullptr},
    {0x09, 0xED, nullptr},
    {0x09, 0xF3, nullptr},
    {0x09, 0xF4, nullptr},
...
const std::vector<Layer> kGCLayers = {
    {0x0C, 0xF4, nullptr}, {0x0C, 0xF5, nullptr}, {0x0C, 0xFA, nullptr},
    {0x0C, 0xFB, nullptr}, {0x0C, 0xFC, nullptr}, {0x0C, 0xFD, nullptr},
};

const std::vector<Layer> kGDLayers = {
    {0x0D, 0xF8, "Assy bot?"}, {0x0D, 0xF9, "Assy top?"},
    {0x0D, 0xFA, "Silk bot?"}, {0x0D, 0xFB, "Silk top?"},
    {0x0D, 0xFC, "Silk bot?"}, {0x0D, 0xFD, "Silk top?"},
};

const std::vector<Layer> kG12Layers = {
    {0x12, 0xF4, nullptr},         {0x12, 0xF5, nullptr},
    {0x12, 0xFA, "Paste bottom?"}, {0x12, 0xFB, "Paste top?"},
    {0x12, 0xFC, "Mask bottom?"},  {0x12, 0xFD, "Mask top?"},
};

const std::map<uint16_t,
               const std::pair<const std::vector<Layer>, const char *>>
    kFixedLayersMap = {
...
class LayerModel : public QAbstractItemModel {
 ...
 public:
  ...
  [[nodiscard]] auto index(int row, int col, const QModelIndex &parent) const
