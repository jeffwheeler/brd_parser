...
void BrdView::loadFile(File<A_MAX> *fs) {
...
void BrdView::drawX01(const x01<A_MAX> *inst, QPainterPath *path) {
...
void BrdView::drawX05(const x05<A_MAX> *inst, QPen *pen_) {
...
void BrdView::drawX14(const x14<A_MAX> *inst, QPen *pen_) {
    if (!onSelectedLayer(inst->subtype, inst->layer) ||
...
void BrdView::drawX15(const x15<A_MAX> *inst, QPainterPath *path) {
...
void BrdView::drawX16(const x16<A_MAX> *inst, QPainterPath *path) {
    path->lineTo(inst->coords[2] / factor, inst->coords[3] / factor);
...
void BrdView::drawX17(const x17<A_MAX> *inst, QPainterPath *path) {
    path->lineTo(inst->coords[2] / factor, inst->coords[3] / factor);
...
// Connectivity (rat)
void BrdView::drawX23(const x23<A_MAX> *inst, QPen *pen) {
    if (!onSelectedLayer(inst->subtype, inst->layer)) {
        return;
    }
...
// Shapes
void BrdView::drawX28(const x28<A_MAX> *inst, QPen *pen) {
    if (!onSelectedLayer(inst->subtype, inst->layer) ||
        fs->is_type(inst->ptr1, 0x2B)) {
        return;
    }
...
void BrdView::drawX2B(const x2B<A_MAX> *inst, QPen *pen) {
    // drawShape(inst->ptr2, pen);
    // drawShape(inst->ptr8, pen);
}

void BrdView::drawX2D(const x2D<A_MAX> *inst, QPen *pen) {
    // Bounding box?
...
void BrdView::drawX30(const x30<A_MAX> *inst, QPen *pen) {
    if (!onSelectedLayer(inst->subtype, inst->layer)) {
        return;
    }
    ...
    */
    ...
    const x31<A_MAX> &str_graphic = fs->get_x31(inst->str_graphic_ptr);
    ...
    const x36_x08<A_MAX> *x36_x08_inst = font_lookup(inst->font.key, *fs);
...
// Pad
void BrdView::drawX32(const x32<A_MAX> *inst, QPen *pen,
                      uint32_t sym_rotation) {
    if (fs->is_type(inst->ptr3, 0x2D)) {
        const x2D<A_MAX> x2D_inst = fs->get_x2D(inst->ptr3);
    ...
    */
    ...
    const x0D<A_MAX> &x0D_inst = fs->get_x0D(inst->ptr5);
    const x1C<A_MAX> &x1C_inst = fs->get_x1C(x0D_inst.pad_ptr);

    QPointF center = QPointF((inst->coords[0] + inst->coords[2]) / 2. / factor,
                             (inst->coords[1] + inst->coords[3]) / 2. / factor);

    // Just draw the first part
    const t13<A_MAX> *first_part = &x1C_inst.parts[0];
    QGraphicsItem *pad;
...
void BrdView::drawX33(const x33<A_MAX> *inst, QPen *pen) {
    ...
    */
    ...
    const x1C<A_MAX> &x1C_inst = fs->get_x1C(inst->ptr4);
    /*
    ...
    */
    ...
    // Just draw the first part
    const t13<A_MAX> *first_part = &x1C_inst.parts[4];
    QGraphicsItem *pad;
...
void BrdView::drawX34(const x34<A_MAX> *inst, QPen *pen) {
    if (!onSelectedLayer(inst->subtype, inst->layer)) {
        return;
    }
...
void BrdView::drawShape(const uint32_t ptr, QPen *pen) {
    ...
    if (fs->is_type(ptr, 0x01)) {
        ...
        const x23<A_MAX> inst = fs->get_x23(ptr);
        drawX23(&inst, pen);
    } else if (fs->is_type(ptr, 0x28)) {
        const x28<A_MAX> inst = fs->get_x28(ptr);
        drawX28(&inst, pen);
        ...
        const x2D<A_MAX> inst = fs->get_x2D(ptr);
        drawX2D(&inst, pen);
    } else if (fs->is_type(ptr, 0x30)) {
        const x30<A_MAX> &inst = fs->get_x30(ptr);
        drawX30(&inst, pen);
        ...
        const x33<A_MAX> &inst = fs->get_x33(ptr);
        drawX33(&inst, pen);
        // drawShape(inst->un1, darkerPen);
        // drawShape(inst->ptr1, darkerPen);
    } else if (fs->is_type(ptr, 0x34)) {
        const x34<A_MAX> &inst = fs->get_x34(ptr);
        drawX34(&inst, pen);
    } else if (fs->is_type(ptr, 0x37)) {
        const x37<A_MAX> &inst = fs->get_x37(ptr);
        drawShape(inst.ptr1, darkerPen);
...
QColor BrdView::customPenColor(uint32_t x05_k, QColor default_) {
    if (fs->is_type(x05_k, 0x05)) {
        const x05<A_MAX> inst = fs->get_x05(x05_k);
        if (onSelectedLayer(inst.subtype, inst.layer)) {
        ...
        const x28<A_MAX> &inst = fs->get_x28(x05_k);
        if (onSelectedLayer(inst.subtype, inst.layer)) {
            return QColorConstants::Svg::palevioletred;
        } else {
            return default_;
        ...
        const x34<A_MAX> &inst = fs->get_x34(x05_k);
        if (onSelectedLayer(inst.subtype, inst.layer)) {
            return QColorConstants::Svg::palevioletred;
        } else {
        ...
        const x30<A_MAX> &inst = fs->get_x30(x05_k);
        if (onSelectedLayer(inst.subtype, inst.layer)) {
...
char *BrdView::netName(uint32_t k) {
    if (fs->is_type(k, 0x28)) {
        const x28<A_MAX> &x28_inst = fs->get_x28(k);
