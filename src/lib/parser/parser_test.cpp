...
TEST(ParseFile, Slugs) {
  std::unique_ptr<File<kAMax>> fs =
      parse_file("../../test/data/slugs/slugs_v2_1.brd");
...
TEST(ParseFile, SmartPlug) {
  ...
  EXPECT_STREQ(
      x03_str_lookup(fs->get_x1B(0x0EAF6CB0).path_str_ptr, *fs).c_str(),
      "@plugmeter_zigbee.plugmeter(sch_1):swdio");
  EXPECT_STREQ(
      x03_str_lookup(fs->get_x1B(0x0EAF6DC8).path_str_ptr, *fs).c_str(), "");
...
TEST(ParseFile, AvalonParsed) {
  std::unique_ptr<File<kAMax>> fs =
      parse_file("../../test/data/avalon/AVALON.brd");
...
TEST(ParseFile, EdaV3) {
  std::unique_ptr<File<kAMax>> fs =
      parse_file("../../test/data/eda_v3/EDA-02319-V3-3.brd");
...
TEST(ParseFile, OpenCellularGbc) {
  std::unique_ptr<File<kAMax>> fs = parse_file(
...
TEST(ParseFile, M1K) {
  std::unique_ptr<File<kAMax>> fs =
      parse_file("../../test/data/m1k/08_037760f.brd");
...
TEST(ParseFile, FmcTlu) {
  std::unique_ptr<File<kAMax>> fs =
      parse_file("../../test/data/fmc_tlu/fmc_tlu_v1f_38.brd");
...
TEST(ParseFile, PllFmHw) {
  std::unique_ptr<File<kAMax>> fs =
      parse_file("../../test/data/pll_fm_hw/FMDEMOD-Long.brd");
