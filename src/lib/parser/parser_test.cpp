...
TEST(ParseFile, Slugs) {
  File<kAMax> fs = *parse_file("../../test/data/slugs/slugs_v2_1.brd");
...
TEST(ParseFile, Parallella) {
  ...
  const x3C<kAMax> &x3C_inst = fs.get_x3C(0x0ACF9230);
...
TEST(ParseFile, SmartPlug) {
  ...
  EXPECT_STREQ(x03_str_lookup(fs.get_x1B(0x0EAF6CB0).path_str_ptr, fs),
               "e.plugmeter(sch_1):swdio");
...
TEST(ParseFile, AvalonParsed) {
  File<kAMax> fs = *parse_file("../../test/data/avalon/AVALON.brd");
...
TEST(ParseFile, EdaV3) {
  File<kAMax> fs = *parse_file("../../test/data/eda_v3/EDA-02319-V3-3.brd");
...
TEST(ParseFile, BeagleYParsed) {
  ...
  EXPECT_STREQ(x03_str_lookup(fs.get_x1B(0x00002337).path_str_ptr, fs),
...
// Magic is 0x00140400
TEST(ParseFile, M1K) {
  File<kAMax> fs = *parse_file("../../test/data/m1k/08_037760f.brd");
...
TEST(ParseFile, CreatableMain) {
  ...
  EXPECT_TRUE(fs.is_type(0x0E7C8635, 0x23));
...
TEST(ParseFile, FmcTlu) {
  File<kAMax> fs = *parse_file("../../test/data/fmc_tlu/fmc_tlu_v1f_38.brd");
...
TEST(ParseFile, PllFmHw) {
  File<kAMax> fs = *parse_file("../../test/data/pll_fm_hw/FMDEMOD-Long.brd");
