...
int main(int argc, char* argv[]) {
    ...
    */
    ...
    while (1) {
        i++;
        if (parsed_file->is_type(k, 0x38)) {
            auto& i = parsed_file->get_x38(k);
            ...
            k = i.next;
        } else if (parsed_file->is_type(k, 0x31)) {
            auto& i = parsed_file->get_x31(k);
            ...
            printf("Found x28 w/ key = 0x %08X, subtype = 0x %02X\n", ntohl(k),
                   i.subtype);
            k = i.ptr5;
            ...
            k = i.un[0];
        } else if (parsed_file->is_type(k, 0x1F)) {
            auto& i = parsed_file->get_x1F(k);
            ...
            k = i.hdr.un1;
        } else if (parsed_file->is_type(k, 0x1D)) {
            auto& i = parsed_file->get_x1D(k);
            printf("Found x1D w/ key = 0x %08X\n", ntohl(k));
            k = i.un[0];
        } else if (parsed_file->is_type(k, 0x1C)) {
            auto& i = parsed_file->get_x1C(k);
            ...
            k = i.next;
        } else if (parsed_file->is_type(k, 0x17)) {
            auto& i = parsed_file->get_x17(k);
            printf("Found x17 w/ key = 0x %08X\n", ntohl(k));
            print_struct(k, *parsed_file, off);
            k = i.next;
        } else if (parsed_file->is_type(k, 0x16)) {
            auto& i = parsed_file->get_x16(k);
            printf("Found x16 w/ key = 0x %08X\n", ntohl(k));
            print_struct(k, *parsed_file, off);
            k = i.next;
            ...
            print_struct(k, *parsed_file, off);
            k = i.next;
        } else if (parsed_file->is_type(k, 0x0E)) {
            auto& i = parsed_file->get_x0E(k);
